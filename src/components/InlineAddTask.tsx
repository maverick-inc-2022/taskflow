...
export default function InlineAddTask({
  ...
  useEffect(() => {
    ...
    (isMobile ? mobileTitleRef : titleRef).current?.focus();
  }, []);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (!projectRef.current?.contains(e.target as Node)) {
        setShowProject(false); setAddingProject(false); setNewProjectLabel("");
      }
    };
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (!ownerRef.current?.contains(e.target as Node)) {
        setShowOwner(false); setAddingPerson(false); setNewPersonName("");
      }
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  ...
  const inputBase = "rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-200";
    ...
    <div className="mx-0.5 my-1 rounded-2xl border border-blue-300 bg-blue-50 shadow-lg ring-1 ring-blue-100">
      ...
      <div className="sm:hidden p-3 space-y-2">
        ...
        <input
          ...
          placeholder="タスク名を入力…"
          className="w-full rounded-xl border border-blue-200 bg-white px-3 py-3 text-base font-medium text-slate-700 outline-none placeholder:text-slate-300 focus:border-blue-400"
        />

        {/* Row: project | date | priority */}
        ...
        <div className="flex items-center justify-between pt-0.5">
          <RepeatSelector
            due={due} repeat={repeat} repeatConfig={repeatConfig}
            onChange={(r, cfg) => { setRepeat(r); if (cfg) setRepeatConfig(cfg); }}
          />
          <div className="flex gap-2">
            <button type="button" onClick={onCancel}
              className="rounded-xl px-3 py-2 text-sm text-slate-500 hover:bg-white/60 transition">
              キャンセル
            </button>
            <button type="button" onClick={commit} disabled={!title.trim()}
              className="rounded-xl bg-blue-600 px-5 py-2 text-sm font-semibold text-white disabled:opacity-40 transition">
              追加
            </button>
          </div>
        </div>
      ...
      <div className="hidden sm:block">
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button type="button" disabled={!newProjectLabel.trim()}
                        onClick={() => { if (!newProjectLabel.trim()) return; onAddProject!(newProjectLabel.trim(), newProjectColor); setAddingProject(false); setNewProjectLabel(""); setShowProject(false); }}
                ...
                {!addingPerson ? (
                  <>
                    <button onClick={() => { setOwner(undefined); setShowOwner(false); }}
                      className={`flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-slate-50 ${!owner ? "font-semibold text-blue-600" : "text-slate-500"}`}>未割当</button>
                    {people.map((p) => (
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <div className="flex gap-2 pt-1">
                      ...
                      <button type="button" disabled={!newPersonName.trim()}
                        onClick={() => { if (!newPersonName.trim()) return; onAddPerson!(newPersonName.trim(), newPersonAvatar); setAddingPerson(false); setNewPersonName(""); setNewPersonAvatar(DEFAULT_AVATAR); setShowOwner(false); }}
                        className="flex-1 rounded-md bg-blue-600 py-1 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-40 transition">追加</button>
                    </div>
                  </div>
                )}
        ...
        <div className="flex items-center justify-between border-t border-blue-100 px-4 py-2">
          <RepeatSelector due={due} repeat={repeat} repeatConfig={repeatConfig}
            onChange={(r, cfg) => { setRepeat(r); if (cfg) setRepeatConfig(cfg); }} />
          <div className="flex items-center gap-2">
            <button type="button" onClick={onCancel}
              className="rounded-lg px-3 py-1.5 text-sm text-slate-500 hover:bg-white/60 transition">キャンセル</button>
            <button type="button" onClick={commit} disabled={!title.trim()}
              className="rounded-lg bg-blue-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-40 transition">追加</button>
          </div>
        </div>
      </div>
