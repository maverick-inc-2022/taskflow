...
interface Props {
  ...
  mainMode: "tasks" | "memos" | "whiteboard";
  onChangeMainMode: (mode: "tasks" | "memos" | "whiteboard") => void;
    ...
    >
      ...
      {!collapsed ? (
        <div className="mb-4 flex gap-0.5 rounded-xl bg-slate-100 p-0.5">
          ...
          <button
            onClick={() => onChangeMainMode("whiteboard")}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-1.5 text-xs font-semibold transition ${
              mainMode === "whiteboard" ? "bg-white text-slate-700 shadow-sm" : "text-slate-400 hover:text-slate-600"
            }`}
          >
            <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
            ボード
          </button>
        </div>
      ) : (
        ...
        <div className="mb-4 flex flex-col gap-1">
          ...
          <button onClick={() => onChangeMainMode("whiteboard")} title="ホワイトボード"
            className={`flex h-8 w-8 items-center justify-center rounded-lg transition ${mainMode === "whiteboard" ? "bg-blue-600 text-white" : "text-slate-400 hover:bg-slate-100"}`}>
            <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
          </button>
        </div>
      )}
