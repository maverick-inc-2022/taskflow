...
const faqs: [string, string][] = [
  ["Googleカレンダーと同期するには？", "設定（歯車アイコン）→「連携」セクションのGoogleカレンダーから「連携する」ボタンをクリックしてアカウントを認証してください。"],
  ["タスクのプロジェクトを変更するには？", "タスクをクリックして右側の詳細パネルを開き、プロジェクトフィールドから変更できます。インライン編集も可能です。"],
  ["お気に入りはどこに表示される？", "タスク右側の☆をクリックするとサイドバーの「お気に入り」に表示されます。"],
  ["完了したタスクを非表示にするには？", "設定（歯車アイコン）→「表示」→「完了したタスクを隠す」をオンにしてください。"],
  ["タスクデータを保存・バックアップするには？", "設定（歯車アイコン）下部の「データをエクスポート」からCSVファイルをダウンロードできます。"],
];
